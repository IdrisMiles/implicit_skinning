...
template< typename T >
void check(T result, char const *const func, const char *const file, int const line, const char *const msg = "")
{
    if (result)
    {
        fprintf(stderr, "CUDA error at %s:%d code=%d(%s) \"%s\" \nUser message: %s\n",
                file, line, static_cast<unsigned int>(result), _cudaGetErrorEnum(result), func, msg);
        DEVICE_RESET
...
#define checkCudaErrorsMsg(val, msg)           check ( (val), #val, __FILE__, __LINE__, msg )
