#include "include/compositionop.h"


CompositionOp::CompositionOp(const unsigned int _dim)
{
    m_dim = _dim;
    m_data = std::shared_ptr<float>(new float[m_dim*m_dim*m_dim]);
}
...
{
    m_dim = _dim;
    m_data.reset(new float[m_dim*m_dim*m_dim]);
}
...
float CompositionOp::TrilinearInterpolate(const float f1, const float f2, const float d)
{
    ...
    // Get values
    float val0 = m_data.get()[Hash(x0, y0, z0)]; //bottom font left
    float val1 = m_data.get()[Hash(x1, y0, z0)]; //bottom front right
    float val2 = m_data.get()[Hash(x0, y1, z0)]; //bottom back left
    float val3 = m_data.get()[Hash(x1, y1, z0)]; //bottom back right

    float val4 = m_data.get()[Hash(x0, y0, z1)];
    float val5 = m_data.get()[Hash(x1, y0, z1)];
    float val6 = m_data.get()[Hash(x0, y1, z1)];
    float val7 = m_data.get()[Hash(x1, y1, z1)];
